...
async def over_token(num_tokens: int, event: NewMessage, prompt: Prompt, filename: str):
    try:
        await event.reply(f"Диалог слишком длинный ({num_tokens} токенов). Начинаю новый 🙂")
        ...
        new_prompt = [{"role": "system", "content": f"Резюме прошлого диалога: {summary}"}]
...
async def get_openai_response(prompt: Prompt, filename: str) -> str:
    ...
    return "⚠️ OpenAI временно недоступен. Попробуйте позже."
